...
namespace G6037599
{
  ...
  template <class T>
  Spawn_to<T>& Spawn_to<T>::operator=(const Spawn_to<T>& t_to_copy)
  {
    copy_from(t_to_copy);
    return *this;
  }
  ...
  template <class T>
  int Spawn_to<T>::get_last_id() const
  {
    return m_monsters_[m_monsters_.size() - 1]->get_id();
  }

  template <class T>
  int Spawn_to<T>::find_hp(const int t_id) const
  {
  ...
  template <class T>
  bool Spawn_to<T>::damaged_n_dies(const int t_monster_id, const int t_damage) const
  {
  ...
  template <class T>
  void Spawn_to<T>::spawn(const int t_monsters)
  {
    for(auto i = 0; i < t_monsters; ++i)
    {
      m_monsters_.push_back(std::make_shared<T>(share_type()));
  ...
  template <class T>
  void Spawn_to<T>::monsters_stronger(const int t_percent) const
  {
  ...
  template <class T>
  COORD Spawn_to<T>::find_pos(const int t_monster_id) const
  {
    REQUIRE(t_monster_id > Map::NO_UNIT);
  ...
  template <class T>
  void Spawn_to<T>::copy_from(const Spawn_to<T>& t_to_copy)
  {
    m_monsters_.clear();
  ...
  template <class T>
  int Spawn_to<T>::find_index(const int t_monster_id) const
  {
    REQUIRE(t_monster_id > Map::NO_UNIT);

    for (auto i = 0; i < m_monsters_.size(); ++i)
    {
